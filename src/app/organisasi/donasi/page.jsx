...
export default function OrgRequestPage() {
  ...
  const handleUpdate = async (e) => {
    ...
    if (res.ok) {
      setRequestList((prev) =>
        prev.map((request) =>
          request.id_request === editData.id_request ? editData : request
        )
      );
      setShowModal(false);
  ...
  const handleDelete = async (id_request) => {
    ...
    if (res.ok) {
      setRequestList((prev) =>
        prev.filter((request) => request.id_request !== id_request)
      );
  ...
  const fetchRequestOrg = async () => {
    try {
      const res = await fetch(
        `/api/requestdonasi/by-org?q=${encodeURIComponent(searchQuery)}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="p-6">
      ...
      <Tabs defaultValue="semua" className="mb-4">
        <TabsList className="flex gap-4">
          {["Semua", "Pending", "Diterima", "Selesai", "Dibatalkan"].map(
            (tab) => (
              <TabsTrigger
                key={tab}
                        ...
                        <div className="absolute left-0 mt-2 bg-white border border-gray-200 rounded shadow-md z-10">
                          <button
                            className={`block w-full px-4 py-2 text-left text-sm ${
                              ["APPROVED", "DONE", "CANCELLED"].includes(
                                item.status_request
                              )
                          ...
                          <button
                            ...
                            disabled={["APPROVED", "DONE"].includes(
                              item.status_request
                            )}
                          >
                            Hapus
