...
export async function GET(req, context) {
    ...
    if (!id_penitip) {
        return NextResponse.json({ error: 'ID penitip tidak ditemukan' }, { status: 400 });
    }
    ...
    try {
        ...
        for (const row of barangRows) {
            ...
            if (row.nama_kategori && !barangMap[row.id_barang].kategori_barang.includes(row.nama_kategori)) {
                barangMap[row.id_barang].kategori_barang.push(row.nama_kategori);
            }
        }

        return NextResponse.json({ barang: Object.values(barangMap) }, { status: 200 });
    } catch (error) {
