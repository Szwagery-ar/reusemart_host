...
export async function GET(request) {
  try {
    ...
    let query = `SELECT bbc.*, p.nama AS pembeli_nama, brg.nama_barang, pen.nama AS nama_penitip
                    ...
                    JOIN penitipanbarang pb ON brg.id_penitipan = pb.id_penitipan
                    JOIN penitip pen ON pb.id_penitip = pen.id_penitip`;
    let values = [];
...
export async function POST(request) {
  try {
    ...
    if (!id_cart || !id_barang) {
      return NextResponse.json(
        { error: "id_cart and id_barang are required!" },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { message: "Bridge entry added successfully!" },
      { status: 201 }
    );
  } catch (error) {
    return NextResponse.json(
      { error: "Failed to add bridge entry" },
      { status: 500 }
    );
  }
}
...
export async function PUT(request) {
  try {
    ...
    if (id_bridge_barang === undefined || is_checkout === undefined) {
      return NextResponse.json(
        { error: "id_bridge_barang and is_checkout are required!" },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { message: "is_checkout updated successfully!" },
      { status: 200 }
    );
  } catch (error) {
    return NextResponse.json(
      { error: "Failed to update is_checkout" },
      { status: 500 }
    );
  }
}
...
export async function DELETE(request) {
  try {
    ...
    if (!id_bridge_barang) {
      return NextResponse.json(
        { error: "id_bridge_barang is required!" },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { message: "Bridge entry deleted successfully!" },
      { status: 200 }
    );
  } catch (error) {
    return NextResponse.json(
      { error: "Failed to delete bridge entry" },
      { status: 500 }
    );
  }
}
