...
export async function GET(request) {
  try {
    ...
    return NextResponse.json(
      { error: "Failed to fetch Pengiriman" },
      { status: 500 }
    );
  }
}
...
export async function POST(request) {
  try {
    const { id_transaksi, id_petugas_kurir, id_alamat, jenis_pengiriman } =
      await request.json();
    ...
    const petugasKurir =
      jenis_pengiriman === "SELF_PICKUP" ? null : id_petugas_kurir;

    // Status pengiriman awal
    let status_pengiriman = "IN_PROGRESS";
    if (transaksi.tanggal_lunas) {
      status_pengiriman =
        jenis_pengiriman === "SELF_PICKUP" ? "PICKED_UP" : "IN_DELIVERY";
    }

    // Ambil lokasi dari alamat
    const [alamatResult] = await pool.query(
    ...
    console.error(error);
    return NextResponse.json(
      { error: "Failed to create Pengiriman" },
      { status: 500 }
    );
  }
}
...
export async function PUT(request) {
  try {
    const { id_pengiriman, status_pengiriman, id_petugas_kurir } =
      await request.json();
    ...
    if (pengiriman.jenis_pengiriman === "COURIER") {
      if (!id_petugas_kurir) {
        return NextResponse.json(
          { error: "id_petugas_kurir is required for COURIER delivery!" },
          { status: 400 }
        );
      }
      ...
      if (["IN_DELIVERY", "PICKED_UP"].includes(status_pengiriman)) {
        ...
        if (ongoingDeliveries.length > 0) {
          return NextResponse.json(
            {
              error:
                "Kurir sedang menangani pengiriman lain yang sedang berlangsung!",
            },
            { status: 400 }
          );
        }
    ...
    console.error(error);
    return NextResponse.json(
      { error: "Failed to update Pengiriman" },
      { status: 500 }
    );
  }
}
...
export async function DELETE(request) {
  try {
    ...
    if (!id_pengiriman) {
      return NextResponse.json(
        { error: "id_pengiriman is required!" },
        { status: 400 }
      );
    }

    const [pengirimanExists] = await pool.query(
      "SELECT * FROM pengiriman WHERE id_pengiriman = ?",
      [id_pengiriman]
    );
    ...
    await pool.query("DELETE FROM pengiriman WHERE id_pengiriman = ?", [
      id_pengiriman,
    ]);
    ...
    return NextResponse.json(
      { error: "Failed to delete Pengiriman" },
      { status: 500 }
    );
  }
}
