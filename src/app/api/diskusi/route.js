  ...
  try {
    ...
    let query = `
            SELECT 
                ...
                d.tanggal, 
                d.isi_diskusi, 
                p.nama AS pembeli_name,
                b.id_barang, 
                b.nama_barang, 
                p.email AS pembeli_email,
                pen.nama AS penitip_name, 
                peg.nama AS pegawai_name
            FROM diskusi d
    ...
    if (search) {
      query += ` WHERE b.nama_barang LIKE ? OR p.nama LIKE ? OR d.isi_diskusi LIKE ?`;
...
export async function POST(request) {
  try {
    const { id_barang, id_pembeli, id_penitip, id_pegawai, isi_diskusi } =
      await request.json();

    if (!id_barang || !isi_diskusi) {
      return NextResponse.json(
        { error: "Masukkan id_barang dan isi_diskusi!" },
        { status: 400 }
      );
    }

    const rolesFilled = [id_pembeli, id_penitip, id_pegawai].filter(
    ...
    if (rolesFilled.length === 0) {
      return NextResponse.json(
        {
          error:
            "minimal ada salah satu dari id_pembeli, id_penitip, or id_pegawai!",
        },
        { status: 400 }
      );
    }
    ...
    if (rolesFilled.length > 1) {
      return NextResponse.json(
        {
          error:
            "Cuman boleh 1 (id_pembeli, id_penitip, or id_pegawai) setiap inputan!",
        },
        { status: 400 }
      );
    }
