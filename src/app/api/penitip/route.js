...
export async function POST(request) {
    try {
        ...
        const file = formData.get("foto_ktp");

        if (!nama || !no_ktp || !no_telepon || !email || !password || !file) {
            return NextResponse.json(
                { error: "Semua field termasuk foto KTP wajib diisi" },
                { status: 400 }
            );
        }
        ...
        if (typeof file.name !== "string") {
            return NextResponse.json(
                { error: "File foto KTP tidak valid" },
                { status: 400 }
            );
        }
        ...
        const { url } = await put(`ktp-${Date.now()}-${file.name}`, fileBuffer, {
            access: "public",
        });

        // Simpan URL (bukan path) ke DB
        const fotoUrl = url;
        ...
        const [exists] = await pool.query(
            "SELECT * FROM penitip WHERE email = ? OR no_ktp = ?",
            [email, no_ktp]
        );
        if (exists.length > 0) {
            return NextResponse.json(
        ...
        await pool.query("UPDATE penitip SET id = ? WHERE id_penitip = ?", [
            id,
            id_penitip,
        ]);

        const [newPenitipRows] = await pool.query(
...
export async function PUT(request) {
    try {
        ...
        if (password && password.trim() !== "") {
            const hashedPassword = await bcrypt.hash(password, 10);
            await pool.query("UPDATE penitip SET password = ? WHERE id_penitip = ?", [
                hashedPassword,
                id_penitip,
            ]);
        }
...
export async function DELETE(request) {
    try {
        ...
        return NextResponse.json(
            { error: "Failed to delete Penitip" },
            { status: 500 }
        );
    }
}
