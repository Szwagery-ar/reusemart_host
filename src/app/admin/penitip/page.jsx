...
export default function AdminPenitipPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        const data = await res.json();
        if (res.ok) {
            setPenitipList(prev => [...prev, { ...formData, id_penitip: data.id_penitip }]);
            setShowModal(false);
            setFormData({ nama: '', email: '', no_ktp: '', no_telepon: '', password: '' });
    ...
    if (error) return <div className="p-6 text-blue-600 text-center align-text text-3xl font-bold">{error}</div>;
    ...
    return (
        <div className="p-6 relative">
            <WithRole allowed={["CS"]}>
                <h1 className="text-2xl font-bold mb-4 text-indigo-700">Data Penitip</h1>
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-left text-sm font-semibold text-gray-600">KTP</th>
                                <th className="px-6 py-3 text-left text-sm font-semibold text-gray-600">Telepon</th>
                                <th className="px-6 py-3 text-left text-sm font-semibold text-gray-600">Badge</th>
                                <th className="px-6 py-3 text-left text-sm font-semibold text-gray-600">Total Barang</th>
                    ...
                    <div className="fixed inset-0 bg-black/20 flex justify-center items-center z-50">
                        <div className="bg-white p-6 rounded-lg w-full max-w-lg">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                <input name="no_ktp" onChange={handleChange} value={formData.no_ktp} className="w-full border px-3 py-2 rounded" placeholder="No. KTP" />
                                ...
                                <input name="password" type="password" onChange={handleChange} value={formData.password} className="w-full border px-3 py-2 rounded" placeholder="Password" />
                    ...
                    <>
                        ...
                        <div className="fixed inset-y-0 right-0 z-50 bg-white w-full max-w-md h-full p-6 shadow-xl transition-transform duration-300">
                            ...
                            <form onSubmit={handleUpdate} className="space-y-4">
                                ...
                                <input name="no_ktp" onChange={handleChange} value={editData.no_ktp} className="w-full border px-3 py-2 rounded" placeholder="No. KTP" />
                                ...
                                <input name="badge_level" onChange={handleChange} value={editData.badge_level} className="w-full border px-3 py-2 rounded" placeholder="Badge Level" />
                                <div className="flex justify-end gap-2">
