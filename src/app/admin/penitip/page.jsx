...
export default function AdminPenitipPage() {
    ...
    const handleGenerateTopSeller = async () => {
        try {
            ...
            if (res.ok) {
                alert(`✅ Top Seller berhasil diperbarui!\n\n📌 ${data.top_seller?.nama || "Tidak ditemukan"} (${data.top_seller?.email})`);
                setTopSeller(data.top_seller || null);

                // refresh data
                const refreshed = await fetch(`/api/penitip?q=${encodeURIComponent(searchQuery)}`);
            ...
            <WithRole allowed={["CS", "Superuser"]}>
                ...
                {topSeller && (
                    <div className="bg-green-50 border border-green-300 text-green-700 p-4 rounded mb-4">
                        <strong>🏆 Top Seller Bulan Ini:</strong><br />
