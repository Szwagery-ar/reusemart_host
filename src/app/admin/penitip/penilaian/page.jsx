...
export default function AdminPenitipPenilaianPage() {
    ...
    useEffect(() => {
        const fetchPenitip = async () => {
            try {
                const res = await fetch(`/api/penitip/penilaian?q=${encodeURIComponent(searchQuery)}`);
                const data = await res.json();
                if (res.ok) {
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                            <tr className="p-5 font-semibold text-white text-sm bg-[radial-gradient(ellipse_130.87%_392.78%_at_121.67%_0.00%,_#26C2FF_0%,_#220593_90%)]">
                                {['ID Penitip', 'ID Donasi', 'Nama', 'Tanggal Donasi'].map((col) => (
                        ...
                        <tbody className="bg-white divide-y divide-gray-100">
                            {penitipList.map((p, index) => (
                                <tr key={index} className="odd:bg-white even:bg-gray-100 hover:bg-gray-200">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{p.id_penitip}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{p.id_donasi}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{p.nama}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{formatTanggal(p.tanggal_donasi)}</td>
