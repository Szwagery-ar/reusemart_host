...
export default function AdminPengirimanPage() {
  ...
  const handleFinish = async (id_transaksi) => {
    try {
      console.log("Test");
      const res = await fetch(`/api/pengiriman/${id_transaksi}/done`, {
        method: "PATCH",
      });
      ...
      {[...filteredList]
        ...
        .map((item) => (
          <div
            ...
            className="grid grid-cols-[100px_180px_160px_160px_160px_120px_140px_100px] px-4 py-2 mt-3 rounded-xl border border-black text-sm"
          >
            ...
            <div className="flex items-center">
              {item.tanggal_terima ? formatDateTime(item.tanggal_terima) : "-"}
            </div>
            <div className="flex items-center">
              ...
              {activeDropdown === item.id_pengiriman && (
                <div className="absolute left-0 mt-2 w-32 bg-white border border-gray-200 rounded shadow-md z-10">
                  ...
                  {(item.status_pengiriman === "IN_DELIVERY" ||
                    ...
                    <button
                      className="block w-full px-4 py-2 text-left text-sm text-blue-600 cursor-pointer hover:bg-gray-100"
                      onClick={() => handleFinish(item.id_pengiriman)}
                    >
                      Selesaikan Pengiriman
                    </button>
                  )}
                  <button className="block w-full px-4 py-2 text-left text-sm text-red-600 cursor-pointer hover:bg-gray-100">
                    Hapus
                  </button>
                </div>
              )}
