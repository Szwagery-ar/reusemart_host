...
export default function AdminDashboardPage() {
  ...
  const lengkapiBulanPenjualan = (rawTable) => {
    ...
    const map = Object.fromEntries(
      rawTable.map((row) => [
        row[0],
        { jumlah_terjual: row[1], jumlah_penjualan: row[2] },
      ])
    );
  ...
  const downloadPenjualanBulananPDF = async (tableData, tahun) => {
    ...
    const totalJumlah = tableData.reduce(
      (sum, d) => sum + (d.jumlah_terjual || 0),
      0
    );
    const totalPenjualan = tableData.reduce(
      (sum, d) => sum + (d.jumlah_penjualan || 0),
      0
    );
    ...
    autoTable(doc, {
      ...
      body: [
        ...
        [
          "Total",
          Math.round(totalJumlah).toLocaleString("id-ID"),
          Math.round(totalPenjualan).toLocaleString("id-ID"),
        ],
      ],
  ...
  const downloadPDFKomisi = (tableData, tahun, bulan = null) => {
    ...
    const bulanNama = bulan
      ? new Date(`${tahun}-${bulan}-01`).toLocaleString("id-ID", {
          month: "long",
        })
      : "Semua Bulan";
    ...
    const totalHarga = tableData.reduce(
      (sum, d) => sum + (d.harga_jual || 0),
      0
    );
    const totalHunter = tableData.reduce(
      (sum, d) => sum + (d.komisi_hunter || 0),
      0
    );
    const totalReusemart = tableData.reduce(
      (sum, d) => sum + (d.komisi_reusemart || 0),
      0
    );
    const totalPenitip = tableData.reduce(
      (sum, d) => sum + (d.bonus_penitip || 0),
      0
    );
    ...
    autoTable(doc, {
      ...
      body: [
        ...
        [
          ...
          "",
          "",
          formatCurrency(totalHunter),
          formatCurrency(totalReusemart),
          formatCurrency(totalPenitip),
        ],
      ],
      styles: { fontSize: 9 },
  ...
  const downloadPDFPenjualanPerKategori = (kategoriTable = [], tahun) => {
    ...
    const totalTerjual = kategoriTable.reduce(
      (sum, row) => sum + Number(row[1] || 0),
      0
    );
    const totalGagal = kategoriTable.reduce(
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <ChartWithTotal
            ...
            chartComponent={
              <BarChartJt
                data={penjualanLengkap.map((d) => ({
                  name: d.bulan,
                  value: d.jumlah_penjualan,
                }))}
              />
            }
            onDownload={() =>
              downloadPenjualanBulananPDF(penjualanLengkap, selectedYear)
            }
          />
        </div>
        {/*                 
        ...
        <div className="grid grid-cols-2 md:grid-cols-2 gap-3">
          <StatCard
            ...
            value={formatCurrency(
              data.penjualan?.reduce((sum, d) => sum + d.value, 0) || 0
            )}
            growth="+21%"
          ...
          <StatCard
            title="Total Komisi"
            value={formatCurrency(
              data.komisi?.reduce((sum, d) => sum + d.value, 0) || 0
            )}
            growth="+14%"
          ...
          <StatCard
            title="Stok Gudang"
            value={`${
              data.stok?.reduce((sum, d) => sum + d.value, 0) || 0
            } barang`}
            growth="-8%"
            icon={<Box size={16} />}
          />
          <StatCard
            ...
            value={`${data.expired?.length || 0} barang`}
            growth="!"
      ...
      <div className="grid grid-cols-2 gap-3">
          ...
          <CardContent className="p-4 overflow-auto">
            <div className="flex justify-between items-center mb-4">
              ...
              <button
                className="bg-indigo-600 text-white p-2 rounded"
                onClick={() =>
                  downloadPDFRequestDonasi(data.reqdonasiPDFTable || [])
                }
              >
                <Download size={20} />
              </button>
        ...
        <ChartWithTotal
          ...
          onDownload={() =>
            downloadPDFPenjualanPerKategori(
              data.kategoriTable || [],
              selectedYear
            )
          }
          ...
          <CardContent className="p-4 overflow-auto h-full flex flex-col">
            ...
            {uniquePenitips.size > 0 ? (
              [...uniquePenitips.entries()].map(([id, nama], i) => (
                ...
                >
                  <span className="text-sm px-2 truncate whitespace-nowrap overflow-hidden">
                    {nama}
                  </span>
                  <button
              ...
              <div className="max-h-64 overflow-y-auto rounded-b-md border">
                {data.komisiTable && data.komisiTable.length > 0 ? (
                  data.komisiTable.map((item, i) => (
                    <div
                      key={i}
                      className="grid grid-cols-[160px_1fr_200px] p-3 border-b text-sm items-center"
                    >
                      <div>{item[0]}</div>
                      <div>{item[1]}</div>
            ...
            <CardContent className="p-4 flex flex-col h-full">
              <div className="flex justify-between items-center mb-4">
                ...
                <button
                  ...
                  onClick={() =>
                    downloadPDFBarangExpired(data.expiredPDFTable || [])
                  }
                >
                  <Download size={20} />
                </button>
