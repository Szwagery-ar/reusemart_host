    ...
    const fetchNota = async () => {
      try {
        const res = await fetch(
          `/api/transaksi-penitipan/nota/${id_penitipan}`
        );
      ...
      <div ref={notaRef}>
        ...
        <div className="border p-4 rounded">
          ...
          <div className="mb-3">
            ...
            <p>
              Tanggal penitipan :{" "}
              {nota?.tanggal_masuk ? formatDateTime(nota.tanggal_masuk) : "-"}
            </p>
            <p>
              Masa penitipan sampai :{" "}
              {nota?.tanggal_expire ? formatDate(nota.tanggal_expire) : "-"}
            </p>
          </div>

          <div className="mb-3">
            <p className="font-semibold">
              Penitip : {nota?.penitip?.id || "-"} /{" "}
              {nota?.penitip?.nama || "-"}
            </p>
          </div>
          ...
          {(nota.barang || []).map((b, i) => (
            <div key={i} className="mb-1">
              ...
              {b.tanggal_garansi && (
                <p>
                  Garansi ON :{" "}
                  {new Date(b.tanggal_garansi).toLocaleString("id-ID", {
                    month: "long",
                    year: "numeric",
                  })}
                </p>
