...
export default function AdminTransaksiPage() {
  ...
  const handleDetailClick = async (transaksi) => {
    ...
    try {
      const res = await fetch(
        `/api/transaksi/manajemen-transaksi/barang-transaksi?id_transaksi=${transaksi.id_transaksi}`
      );
      const data = await res.json();
      if (res.ok) {
  ...
  return (
    <div className="p-6">
      <h1 className="text-4xl font-[Montage-Demo] mb-4">Manajemen Transaksi</h1>
        ...
        <div>
          ...
          {filteredTransaksi.map((item) => (
            <div
              ...
              className="grid grid-cols-[80px_1fr_200px_140px_140px_140px_100px_80px] px-4 py-2 mt-3 rounded-xl border border-black text-sm"
            >
              ...
              <div className="flex items-center">
                {item.tanggal_lunas ? formatDate(item.tanggal_lunas) : "-"}
              </div>
              <div className="flex items-center">
              ...
              <div className="bg-white w-full h-full max-w-2xl overflow-auto">
                ...
                <div className="p-6">
                  <div className="space-y-2 text-sm">
                    <p>
                      <strong>No Nota:</strong> {selectedTransaksi.no_nota}
                    </p>
                    <p>
                      <strong>Nama Pembeli:</strong> {selectedTransaksi.nama}
                    </p>
                    <p>
                      <strong>Jenis Pengiriman:</strong>{" "}
                      {selectedTransaksi.jenis_pengiriman || "-"}
                    </p>
                    <p>
                      <strong>Tanggal Pesan:</strong>{" "}
                      {formatDate(selectedTransaksi.tanggal_pesan)}
                    </p>
                    <p>
                      <strong>Tanggal Lunas:</strong>{" "}
                      {selectedTransaksi.tanggal_lunas
                        ? formatDate(selectedTransaksi.tanggal_lunas)
                        : "-"}
                    </p>
                    <p>
                      <strong>Harga Awal:</strong>{" "}
                      {formatRupiah(selectedTransaksi.harga_awal)}
                    </p>
                    <p>
                      <strong>Ongkos Kirim:</strong>{" "}
                      {formatRupiah(selectedTransaksi.ongkos_kirim)}
                    </p>
                    <p>
                      <strong>Diskon:</strong>{" "}
                      {formatRupiah(selectedTransaksi.diskon)}
                    </p>
                    <p>
                      <strong>Harga Akhir:</strong>{" "}
                      {formatRupiah(selectedTransaksi.harga_akhir)}
                    </p>
                    <p>
                      <strong>Status:</strong>{" "}
                      {getStatusLabel(selectedTransaksi.status_transaksi)}
                    </p>
