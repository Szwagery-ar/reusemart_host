...
export default function AdminOrganisasiPage() {
    ...
    const fetchOrganisasi = async () => {
      try {
        const res = await fetch(
          `/api/organisasi?q=${encodeURIComponent(searchQuery)}`
        );
  ...
  const handleUpdate = async (e) => {
    ...
    if (res.ok) {
      setOrganisasiList((prev) =>
        prev.map((org) =>
          org.id_organisasi === editData.id_organisasi ? editData : org
        )
      );
  ...
  const handleDelete = async (id_organisasi) => {
    ...
    if (res.ok) {
      setOrganisasiList((prev) =>
        prev.filter((org) => org.id_organisasi !== id_organisasi)
      );
          ...
          <>
            ...
            <div className="fixed inset-y-0 right-0 z-50 bg-white w-full max-w-md h-full shadow-xl transition-transform duration-300">
              ...
              <form
                ...
                className="flex flex-col mb-4 gap-6 p-6"
              >
                <div className="">
