...
export default function CartPage() {
        ...
        const fetchCartData = async () => {
            try {
                ...
                const mergedItems = bridgeItems.map((item) => {
                    const barang = barangData.barang.find((b) => b.id_barang === item.id_barang);
                    return {
                        ...item,
                        nama_barang: barang?.nama_barang,
                        harga_barang: barang?.harga_barang,
                        src_img: barang?.gambar_barang?.[0]?.src_img || "/images/default-image.jpg",
                    };
                });
    ...
    const toggleSelectGroup = (penitip, items) => {
        const allSelected = items.every(item => selectedItems.includes(item.id_bridge_barang));
    ...
    const handleRemove = async (id_bridge_barang) => {
        ...
        if (res.ok) {
            setCartItems((prev) => prev.filter((item) => item.id_bridge_barang !== id_bridge_barang));
            ...
            <div className="px-20 my-25">
                ...
                {cartItems.length === 0 ? (
                    <div className="text-center mt-12">
                        <img src="/images/empty-cart.png" alt="Kosong" className="mx-auto w-40 mb-4" />
                            ...
                            <label className="flex items-center gap-2">
                                <div className="flex items-center gap-2">
                                    <div className="relative">
                                        ...
                                        >
                                            {selectedItems.length === cartItems.length && (
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" fill="white" className="w-3 h-3">
                                    ...
                                    <label className="flex items-center gap-2 text-lg font-bold mb-3">
                                        <div className="relative">
                                            <input
                                                ...
                                                checked={items.every(item => selectedItems.includes(item.id_bridge_barang))}
                                                className="sr-only"
                                            />
                                            <div
                                                className={`w-5 h-5 rounded-sm border-2 border-black flex items-center justify-center transition p-1
                                                        ${items.every(item => selectedItems.includes(item.id_bridge_barang)) ?
                                                        'bg-[radial-gradient(ellipse_130.87%_130.78%_at_101.67%_0.00%,_#26C2FF_0%,_#220593_90%)] border-none'
                                                        :
                                                        'border-black bg-white'
                                                    }`}
                                            ...
                                            <div className="flex gap-4 items-center">
                                                <label className="flex items-center gap-2">
                                                    <div className="relative">
                                                        <input
                                                            type="checkbox"
                                                            onChange={() => toggleSelectItem(item.id_bridge_barang)}
                                                            checked={selectedItems.includes(item.id_bridge_barang)}
                                                            className="sr-only"
                                                        />
                                                        <div
                                                            className={`w-5 h-5 rounded-sm border-2 border-black flex items-center justify-center transition p-1
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <p className="font-bold text-xl mt-2">{formatRupiah(item.harga_barang)}</p>
                                                </div>

                                                <ReduseButton onClick={() => handleRemove(item.id_bridge_barang)}>
                        ...
                        <div className="bg-white shadow-md rounded-xl p-4 mt-6">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <input
                                    ...
                                    onChange={(e) => {
                                        ...
                                        const maxPoin = Math.min(user?.poin_loyalitas || 0, Math.floor(totalHarga / 100));
                                        setPoinDigunakan(isNaN(val) ? 0 : Math.max(0, Math.min(val, maxPoin)));
                                    }}
                            ...
                            {poinDigunakan > 0 && (
                                <div className="flex justify-between">
                                    ...
                                    <span className="text-green-600">- {formatRupiah(potonganPoin)}</span>
                                </div>
                            )}
